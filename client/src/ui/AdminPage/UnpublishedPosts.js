...
class UnpublishedPosts extends React.Component {
    ...
    fetchData = async () => {
        try {
            ...
            const urlSettings = new URL(`${server}/setting`); // TODO: handle dev vs prod enviroments
            ...
            this.setState({
                isLoaded: true,
                posts: resultPosts.posts,//resultPost.posts.filter(post => !post.published),
            ...
            <div style={approvalStyle}>
                <MaterialTable
                    ...
                    actions={[
                        ...
                        rowData => ({
                            tooltip: 'Delete Selected Post',
                            icon: TableIcons.Delete,
                            onClick: oldData =>
                                new Promise(resolve => {
                                    setTimeout(() => {
                                        console.log(rowData);
                                        const url = new URL(`${server}/post/` + rowData.id);
                                        fetch(url, {
                                            method: 'DELETE',
                                            headers: {
                                                'Content-Type': 'application/json',
                                            ...
                                            this.setState(prevState => {
                                                const posts = [...prevState.posts];
                                                posts.splice(posts.indexOf(rowData), 1);
                                                return { ...prevState, posts };
                                            });
                                            resolve();
                                        }).catch((error) => {
                            ...
                            hidden: this.props.userRole!=='ADMIN'
                        }),
                    ]}
                />
            </div>
